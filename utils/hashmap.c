 ...
 */
map_t hashmap_new() {
  hashmap_map* m = (hashmap_map*) malloc_c(sizeof(hashmap_map));

  m->data = (hashmap_element*) calloc_c(INITIAL_SIZE, sizeof(hashmap_element));
...
int hashmap_rehash(map_t in){
  ...
  hashmap_element* temp = (hashmap_element *)
    calloc_c(2 * m->table_size, sizeof(hashmap_element));
...
any_t hashmap_get_def(map_t in, const char* key, any_t def) {
  any_t ret;
  if (hashmap_get(in, key, &ret) == MAP_OK) {
    return ret;
  } else {
    return def;
  }
